...
export default function Example({ route, navigation }) {
    const { folio,id_cita } = route.params;
    ...
    const fetchUser = async () => {
        const data = { 'clave': folio, 'id_cita': id_cita };
        const response = await fetch('https://www.sit-zac.org.mx/guardar_servicio_mostrar_cita_publica', {
            method: 'post',
    ...
    const cancelarCita = async () => {
        const data = { 'clave': folio, 'id_cita': id_cita };
        const response = await fetch('https://www.sit-zac.org.mx/guardar_servicio_cancelar_cita_publica', {
            method: 'post',
            body: JSON.stringify(data),
            headers: {
                'Content-Type': 'aplication/json, image/png',
                    ...
                    >
                        ...
                        <Stack p="4" space={3}>
                            ...
                            {datos.estado === "CONFIRMADA" &&
                                <Stack mb="2">
                                    <Button
                                        onPress={() => Linking.openURL('https://sit-zac.org.mx/imprimir_acuse_cita_publica/' + datos.folio)}
                                        variant="subtle"
                                        mb="2"
                            ...
                            {datos.estado === "CANCELADA" &&
                                <Button
                                    onPress={() => Linking.openURL('https://sit-zac.org.mx/imprimir_acuse_cancelacion_publica/' + datos.folio)}
                                    variant="subtle"
                                    mb="2"
                                    endIcon={<Icon as={Ionicons} name="cloud-download-outline" size="sm" />}
                                >
